...
public interface IRecurringRuleController {
    ...
    @Operation(
            ...
            description = "Returns all recurring rules owned by the authenticated user, paginated.",
            security = @SecurityRequirement(name = "bearerAuth"),
            responses = {
                    @ApiResponse(
                            responseCode = "200",
                            description = "Recurring rules listed successfully.",
                            content = @Content(
                                    mediaType = "application/json",
                                    examples = @ExampleObject(
                                            name = "Page with one monthly income rule.",
                                            value = """
                                                    {
                                                        "content": [
                                                            {
    ...
    ResponseEntity<PageResponseDTO<RecurringRuleResponseDTO>> findAll(
            @Parameter(hidden = true)
            @AuthenticationPrincipal(expression = "user") User user,
    ...
    @Operation(
            ...
            description = "Permanently deletes the recurring rule with the given ID. Only rules owned by the authenticated user can be deleted.",
            security = @SecurityRequirement(name = "bearerAuth"),
            responses = {
                    @ApiResponse(responseCode = "204", description = "Recurring rule deleted successfully."),
                    @ApiResponse(
                            responseCode = "404",
                            description = "Recurring rule not found or does not belong to the authenticated user.",
                            content = @Content(
    ...
    @Operation(
            summary = "Toggle a recurring rule",
            description = """
                    Activates an inactive rule or deactivates an active rule.
                    Inactive rules are skipped by the daily scheduler — use this to pause a rule without deleting it.
                    The isActive state is flipped on each call.
                    """,
            security = @SecurityRequirement(name = "bearerAuth"),
            responses = {
                    @ApiResponse(
    ...
    ResponseEntity<RecurringRuleResponseDTO> toggle(
            @Parameter(hidden = true)
            @AuthenticationPrincipal(expression = "user") User user,
            @Parameter(description = "Recurring rule ID", required = true, example = "a1b2c3d4-0000-1111-2222-333344445555")
            @PathVariable UUID id
    ...
    @Operation(
            ...
            description = """
                    Replaces all mutable fields of the recurring rule.
                    The isActive state is preserved — use the toggle endpoint to change it.
                    Duplicate rules (same owner, category, price, date and description) are rejected.
                    The category must belong to the authenticated user or be a global category.
                    """,
            security = @SecurityRequirement(name = "bearerAuth"),
            responses = {
                    @ApiResponse(
                            ...
                            content = @Content(
                                    ...
                                    schema = @Schema(implementation = RecurringRuleResponseDTO.class),
                                    examples = @ExampleObject(
                                            name = "Monthly income rule updated.",
                                            value = """
                                                    {
                                                        "id": "a1b2c3d4-0000-1111-2222-333344445555",
                    ...
                    @ApiResponse(
                            responseCode = "404",
                            description = "Recurring rule or category not found.",
                            content = @Content(
                                    mediaType = "application/json",
                                    schema = @Schema(implementation = StandardException.class),
                                    examples = @ExampleObject(
    ...
    ResponseEntity<RecurringRuleResponseDTO> update(
            @Parameter(hidden = true)
            @AuthenticationPrincipal(expression = "user") User user,
            @Parameter(description = "Recurring rule ID", required = true, example = "a1b2c3d4-0000-1111-2222-333344445555")
            @PathVariable UUID id,
            @io.swagger.v3.oas.annotations.parameters.RequestBody(
                    description = """
                            All mutable fields are required. The isActive state is not affected.
                            Validation rules:
                                - transactionDate: required. Day-of-month used for MONTHLY; day-of-week used for WEEKLY.
                                - price: required, must be positive.
                                - description: required, cannot be blank.
                    ...
                    required = true,
                    content = @Content(
                            schema = @Schema(implementation = RecurringRuleUpdateDTO.class),
                            examples = @ExampleObject(
                                    name = "Update monthly income rule.",
                                    value = """
                                            {
                                                "transactionDate": "2026-07-01T00:00:00",
                                                "price": 6000.00,
